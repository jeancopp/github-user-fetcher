...
import { findUsersByLocationAndTechnology } from '../../src/repository/UserRepository';
...
describe('ListUserService', () => {
    ...
    it('Should print that no users was found', async () => {
        (findUsersByLocationAndTechnology as jest.Mock).mockResolvedValueOnce([]);
    ...
    it('Should print all the found users', async () => {
        //given
        (findUsersByLocationAndTechnology as jest.Mock).mockResolvedValueOnce(users);
        ...
        users.forEach(({name, location, login}) => {
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining(login));
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining(name || 'No Name'));
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining(location || 'Unknown'));
        })
    });
